...
The Jensen-Shannon Divergence (JSD) is a symmetric and always finite measure of similarity between probability distributions. It is built from the [Kullback-Leibler Divergence](/atlas/math/probability/kullback-leibler-divergence) and is widely used when KL is too brittle or asymmetric.
## Concepts
- **Probability Distribution:** A function that assigns probabilities to outcomes or events.
- **Kullback-Leibler Divergence:** An asymmetric divergence that measures how one distribution differs from another. See [Kullback-Leibler Divergence](/atlas/math/probability/kullback-leibler-divergence).
- **Entropy:** A measure of uncertainty in a probability distribution. See [Entropy](/atlas/math/probability/entropy).
...
### Motivation
The [KL divergence](/atlas/math/probability/kullback-leibler-divergence) is useful, but it has two major drawbacks:
- **Asymmetry**: $D_{KL}(P\|Q) \neq D_{KL}(Q\|P)$
...
where $H(\cdot)$ is the Shannon entropy, see [Entropy](/atlas/math/probability/entropy).
